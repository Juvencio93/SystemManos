...
- **Relatório Interno:** `lovable-error-reporting.ts` para capturar falhas de runtime no frontend sem dependências externas.
...
## 2. Sentry (Opcional)
A estrutura para o Sentry está preparada em `src/lib/lovable-error-reporting.ts`, mas **não está configurada**. 
