-- Canonical routing identity for chat conversations.
-- Existing rows remain compatible; new and reused rows receive a stable key.
ALTER TABLE public.conversations
  ADD COLUMN IF NOT EXISTS routing_key text;

CREATE UNIQUE INDEX IF NOT EXISTS conversations_company_routing_key_uidx
  ON public.conversations (coalesce(company_id, '00000000-0000-0000-0000-000000000000'::uuid), routing_key)
...
BEGIN
  IF jsonb_typeof(p_participants) <> 'array' OR jsonb_array_length(p_participants) = 0 THEN
    RAISE EXCEPTION 'At least one chat participant is required';
  END IF;

  v_derived_key := public.chat_participant_routing_key(p_participants);
  IF v_key IS NULL OR v_key <> v_derived_key THEN
    v_key := v_derived_key;
  END IF;
  IF v_key IS NULL OR length(v_key) > 500 THEN
    RAISE EXCEPTION 'Invalid chat routing key';
  END IF;

  -- Serialize creation/reuse for the same logical conversation.
  PERFORM pg_advisory_xact_lock(hashtextextended(
    coalesce(p_company_id::text, 'platform') || ':' || v_key, 0
  ));

  SELECT c.id INTO v_id
  FROM public.conversations c
  WHERE c.company_id IS NOT DISTINCT FROM p_company_id
    AND c.routing_key = v_key
  ORDER BY c.created_at ASC
  LIMIT 1;

  -- Compatibility lookup for legacy rows that predate routing_key.
  IF v_id IS NULL THEN
    SELECT c.id INTO v_id
    FROM public.conversations c
    WHERE c.company_id IS NOT DISTINCT FROM p_company_id
      AND c.routing_key IS NULL
      AND EXISTS (
        SELECT 1
  ...
  IF v_id IS NOT NULL THEN
    ...
    SET routing_key = v_key
    WHERE id = v_id AND routing_key IS NULL;
    ...
    FOR v_participant IN SELECT * FROM jsonb_array_elements(p_participants) LOOP
      ...
      SELECT v_id,
        nullif(v_participant->>'profile_id', '')::uuid,
        nullif(v_participant->>'branch_id', '')::uuid,
        v_participant->>'participant_type'
      WHERE NOT EXISTS (
        SELECT 1 FROM public.conversation_participants cp
        WHERE cp.conversation_id = v_id
          AND cp.profile_id IS NOT DISTINCT FROM nullif(v_participant->>'profile_id', '')::uuid
          AND cp.branch_id IS NOT DISTINCT FROM nullif(v_participant->>'branch_id', '')::uuid
          AND cp.participant_type = v_participant->>'participant_type'
      );
    END LOOP;
  ...
  INSERT INTO public.conversations(company_id, routing_key, last_message_at)
  VALUES (p_company_id, v_key, now())
