-- 1. Add history_cleared_at to conversation_user_preferences
ALTER TABLE public.conversation_user_preferences ADD COLUMN IF NOT EXISTS history_cleared_at timestamptz NULL;

-- 2. Update RPC to be canonical and handle visibility/history clearing correctly
...
BEGIN
    ...
    -- We ignore hidden_at here because we want the CANONICAL one, even if hidden.
    ...
    ORDER BY c.created_at ASC -- Take the oldest one as canonical if multiple exist (pre-consolidation)
    LIMIT 1;
