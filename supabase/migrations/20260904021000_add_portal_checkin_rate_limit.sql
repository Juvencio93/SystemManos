-- Server-only limiter for the anonymous public captive-portal check-in.
CREATE TABLE IF NOT EXISTS public.portal_checkin_rate_limits (
...
ALTER TABLE public.portal_checkin_rate_limits ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.portal_checkin_rate_limits FROM PUBLIC, anon, authenticated;
GRANT ALL ON public.portal_checkin_rate_limits TO service_role;
...
COMMENT ON FUNCTION public.consume_portal_checkin_rate_limit(text, integer, integer)
IS 'Internal server-only limiter for anonymous captive-portal check-ins.';
