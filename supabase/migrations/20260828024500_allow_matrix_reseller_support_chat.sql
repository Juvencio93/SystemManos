-- Allow matrices and branches to contact their owning reseller as Support Técnico.
...
DECLARE me record; peer record; parts jsonb; k text; BEGIN
  SELECT * INTO me FROM public.user_roles WHERE user_id=auth.uid() ORDER BY CASE role::text WHEN 'adm' THEN 1 WHEN 'revenda' THEN 2 WHEN 'matriz' THEN 3 ELSE 4 END LIMIT 1;
  SELECT * INTO peer FROM public.user_roles WHERE user_id=p_recipient_profile_id ORDER BY CASE role::text WHEN 'adm' THEN 1 WHEN 'revenda' THEN 2 WHEN 'matriz' THEN 3 ELSE 4 END LIMIT 1;
  ...
  IF NOT ((me.role::text='adm' AND peer.role::text IN ('matriz','filial','revenda')) OR
    ...
    (me.role::text='matriz' AND (peer.role::text='adm' OR (peer.role::text='filial' AND peer.company_id=me.company_id) OR (peer.role::text='revenda' AND EXISTS (SELECT 1 FROM public.companies c WHERE c.id=me.company_id AND c.reseller_id=peer.reseller_id AND c.status='ativa')))) OR
    (me.role::text='filial' AND (peer.role::text='adm' OR (peer.role::text IN ('matriz','filial') AND peer.company_id=me.company_id) OR (peer.role::text='revenda' AND EXISTS (SELECT 1 FROM public.companies c WHERE c.id=me.company_id AND c.reseller_id=peer.reseller_id AND c.status='ativa')))))
  ...
  IF me.role::text='revenda' OR peer.role::text='revenda' THEN
    parts:=jsonb_build_array(jsonb_build_object('participant_type',CASE WHEN me.role::text='revenda' THEN 'revenda' ELSE 'support' END,'profile_id',me.user_id,'branch_id',me.branch_id),jsonb_build_object('participant_type',CASE WHEN peer.role::text='revenda' THEN 'revenda' ELSE 'support' END,'profile_id',peer.user_id,'branch_id',peer.branch_id));
  ELSE
